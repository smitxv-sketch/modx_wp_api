...
export const SeoReportTemplate = forwardRef<HTMLDivElement, SeoReportTemplateProps>(({ data }, ref) => {
  return (
    <div ref={ref} className="p-8 bg-white text-slate-900 font-sans max-w-5xl mx-auto print:p-0">
      {/* Header */}
      <div className="mb-8 border-b pb-6">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">{data.title}</h1>
        <p className="text-slate-500">Дата формирования: {data.date}</p>
      </div>

      {/* Summary Dashboard */}
      <div className="mb-12 page-break-inside-avoid">
        <h2 className="text-2xl font-semibold text-slate-800 mb-6">{data.summary.title}</h2>
      ...
      {data.sections.map((section, idx) => (
        <div key={section.id} className="mb-12" style={{ pageBreakInside: 'auto' }}>
          <div className="page-break-inside-avoid">
            <h2 className="text-xl font-semibold text-slate-800 mb-2 border-b pb-2">{section.title}</h2>
            {section.description && (
          ...
          {section.table && section.table.rows.length > 0 && (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="bg-slate-100/60 backdrop-blur-md border-b-2 border-slate-200">
                ...
                <tbody>
                  {section.table.rows.map((row, rIdx) => (
                    <tr key={rIdx} className="border-b border-slate-100 hover:bg-slate-50" style={{ pageBreakInside: 'avoid' }}>
                      {section.table.columns.map((col) => (
                        <td key={col.key} className="p-3 text-slate-800 align-top break-words max-w-xs">
                          {row[col.key]}
                        </td>
                      ))}
                    </tr>
          ...
          {section.table && section.table.rows.length === 0 && (
            <p className="text-slate-500 italic text-sm">Нет данных для отображения</p>
          )}
        </div>
      ))}
    </div>
